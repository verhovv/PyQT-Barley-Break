...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.record = QLabel(f'Рекорд: {record()}', self)
        self.record.resize(150, 25)
        self.record.setFont(MAIN_FONT)
        self.record.move(self.time.x(), self.time.y() + 15)
    ...
    def make_plates(self):
        ...
        self.time.setText(f'Время: 0 секунд')
        self.record.setText(f'Рекорд: {record(plates_width * plates_height)}')
